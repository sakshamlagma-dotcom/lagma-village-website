  ...
  function build() {
    ...
    page.innerHTML = `
      <main class="chat-app" aria-label="TODO AI chat">
        ...
        <div class="chat-workspace"><aside class="tools-rail" aria-label="Chat tools"><button class="rail-action active" type="button">Chat</button><button class="rail-action" type="button">Write</button><button class="rail-action" type="button">Study</button><button class="rail-action" type="button">Ideas</button><button class="rail-action" type="button">Translate</button></aside><section class="chat-scroll" id="chatScroll" aria-live="polite"><div class="chat-content" id="chatMessages"></div><button class="jump-button" id="jumpButton" type="button">↓ New messages</button></section><aside class="tools-panel" id="toolsPanel" aria-label="Assistant tools"><section class="live-card"><span class="tool-kicker">VOICE ASSISTANT</span><h2>Talk Live</h2><p>Have a natural conversation with TODO AI.</p><button class="live-start-button" type="button" data-open-live>Start voice chat</button></section><section class="quick-actions"><div class="panel-heading"><h2>Quick Actions</h2><button type="button" aria-label="More quick actions" disabled>More</button></div><div class="quick-action-grid"><button type="button" data-prompt="Create an image prompt for me"><span class="tool-icon">Image</span><span>Create Image</span></button><button type="button" data-prompt="Generate clean code for this task"><span class="tool-icon">Code</span><span>Generate Code</span></button><button type="button" data-prompt="Write polished content for me"><span class="tool-icon">Write</span><span>Write Content</span></button><button type="button" data-prompt="Give me five creative ideas"><span class="tool-icon">Ideas</span><span>Get Ideas</span></button></div></section><section class="recent-tools"><div class="panel-heading"><h2>Recent Chats</h2><button id="clearRecentButton" type="button">Clear all</button></div><div id="recentToolsList"></div></section></aside></div>
        ...
        <aside class="history-drawer" id="historyDrawer" aria-hidden="true"><div class="drawer-brand"><span class="chat-brand-mark">T</span><div><strong>TODO AI</strong><small>Think. Ask. Create.</small></div><button class="chat-icon-button drawer-menu-button" type="button" aria-label="Toggle navigation">Menu</button></div><nav class="drawer-nav" aria-label="Assistant navigation"><button class="active" type="button" data-drawer-action="chat">Chat</button><button type="button" data-drawer-action="voice">Voice Chat</button><button type="button" data-drawer-action="create">Create</button><button type="button" data-drawer-action="history">History</button><button type="button" data-drawer-action="settings">Settings</button></nav><div class="drawer-head"><strong>Recent conversations</strong><button class="chat-icon-button" id="closeHistoryButton" type="button" aria-label="Close history">×</button></div><button class="new-chat-button" id="newChatButton" type="button">＋ New conversation</button><label class="history-search"><span aria-hidden="true">⌕</span><input id="historySearch" type="search" placeholder="Search chats" aria-label="Search chat history"></label><div id="historyList"></div><div class="premium-promo"><span>TODO AI Pro</span><strong>Unlock your creative flow</strong><button type="button" disabled>Explore plan</button></div><div class="drawer-footer"><button type="button" id="drawerThemeButton">Dark mode</button><span>TODO AI v1.0</span></div></aside>
