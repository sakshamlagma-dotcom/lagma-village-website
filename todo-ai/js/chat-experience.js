(() => {
    ...
    page.innerHTML = `
      <main class="chat-app" aria-label="TODO AI chat">
        <header class="chat-header">
          <button class="chat-icon-button" id="historyButton" type="button" aria-label="Open chat history" aria-expanded="false">☰</button>
          <div class="chat-brand"><span class="chat-brand-mark">T</span><div><strong>TODO AI</strong><span>Think. Ask. Create.</span></div></div>
          <div class="chat-header-actions"><span class="connection-status" id="connectionStatus"><i></i><span>Online</span></span><button class="chat-icon-button" id="themeButton" type="button" aria-label="Toggle theme">☾</button></div>
        </header>
        <section class="chat-scroll" id="chatScroll" aria-live="polite"><div class="chat-content" id="chatMessages"></div><button class="jump-button" id="jumpButton" type="button">↓ New messages</button></section>
        ...
        <aside class="history-drawer" id="historyDrawer" aria-hidden="true"><div class="drawer-head"><strong>Chat history</strong><button class="chat-icon-button" id="closeHistoryButton" type="button" aria-label="Close history">×</button></div><button class="new-chat-button" id="newChatButton" type="button">＋ New conversation</button><div id="historyList"></div></aside>
  ...
  function renderMessages() {
    ...
    if (!state.messages.length) {
      ...
      box.innerHTML = state.messages.map((message, index) => `<article class="message-row ${message.role === 'user' ? 'user-message' : 'assistant-message'}" data-index="${index}"><div class="message-avatar">${message.role === 'user' ? 'You' : 'T'}</div><div class="message-column"><div class="message-bubble">${message.role === 'assistant' ? markdown(message.text) : escapeHtml(message.text).replace(/\n/g, '<br>')}</div><div class="message-actions"><span>${message.role === 'assistant' ? 'TODO AI' : 'You'}</span>${message.role === 'assistant' ? `<button type="button" data-action="speak">🔊</button>` : ''}<button type="button" data-action="copy">Copy</button></div></div></article>`).join('');
  ...
  function renderHistory() {
    ...
    if (!list) return;
    list.innerHTML = state.sessions.length ? state.sessions.map((session) => `<button type="button" class="history-item ${session.id === state.sessionId ? 'active' : ''}" data-session="${session.id}"><span>${escapeHtml(session.title)}</span><small>${new Date(session.updatedAt).toLocaleDateString()}</small></button>`).join('') : '<p class="empty-history">Your conversations will appear here.</p>';
